/***************************************************************
Generated by Eclipse
Version JAVA SE 1.7

LockGameRequestController.java : implementation file
This is a controller class for sending  lock game request to server. 

History:
1 Nov, 2016	--- Zhe Lyu @1
- Created.

*****************************************************************/

package client.controller;

import xml.Message;
import client.model.GameRoom;
import client.view.Application;

public class LockGameRequestController {
	Application app;
	GameRoom model;

	public LockGameRequestController(Application app , GameRoom model) {

		this.app = app;

		this.model = model;
	}

	/** 
	 * Make the request to lock game on the server and wait for response. 
	 */
	public void process() {
		// send the request to lock the game.
		//tring xmlString = Message.requestHeader() + "<lockGameRequest gameId='sample'/></request>";
		Message m = new Message (toString());
		System.out.println(m);
		// Request the lock (this might not succeed).
		app.getServerAccess().sendRequest(m);
	}
	/**
	 * Create the XML Message: lockGameRequest
	 */
	public String toString() {
		String s=model.getGameId();
		return new XmlStringBuilder("lockGameRequest").add("gameId",s).finish()
				.toString();
	}
}
